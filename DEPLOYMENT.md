...
- Gruppinformation för **Börja spela** hämtas live från check-in (`GET /api/public/signup-groups`: `name`, `description`, `info`, `forTrialSignup`, `sortOrder`). Redigera texterna i check-in-admin. `public/signup-groups.json` är bara fallback om API:t inte svarar.
